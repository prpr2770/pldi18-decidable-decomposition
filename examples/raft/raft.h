...
class raft {
  public:
    ...
    int last_ix;
    bool non_empty;
        size_t __hash() const { return hash_space::hash<int>()(last_ix)+hash_space::hash<bool>()(non_empty);}
    };
    struct node_term {
    int _term;
    ...
    virtual commited_ix ext__commited_ix__wrap(int maxix);
    ...
    virtual void ext__commited_ix__update_ghost_by(const node& n, int maxix, int maxt, const nset& q);
...
inline bool operator ==(const raft::commited_ix &s, const raft::commited_ix &t){
    return ((s.last_ix == t.last_ix) && (s.non_empty == t.non_empty));
}
